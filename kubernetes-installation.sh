...
python query_csv.py localhost "sum(rate(container_cpu_usage_seconds_total{pod=\"${ingress_nginx_controller_pod_name}\"}[10s])) by (pod_name) * 100" "$start_time" "$end_time" query_cpu.csv
